...
$page_header = "Customer Edit ";
    ...
    if (isset($_POST['update'])){
        ...
        if (!empty($name) && !empty($address) && !empty($phone) && !empty($email) && $status != null){
            $sql = "UPDATE customers SET `name` = '$name', `address` = '$address', `number` = '$phone', `email` = '$email', `status` = '$status' WHERE `id` = '$id'";
            if($db->query($sql)){
                ...
                <form role="form" method="POST" action="">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" value="<?= $data['number']?>" name="number" class="form-control" id="exampleInputEmail1" placeholder="Enter Phone Number">
                        </div>
